  ...
  module Normalizer
    class NormalizerTest < Minitest::Test
      def test_normalize
        ...
        assert_equal "abcdefghijklmnopqrstuvwxyz", Neologdish::Normalizer.normalize("ａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚ")
        assert_equal "!\"\#$%&'()*+,-./:;<>?@[¥]^_`{|}", Neologdish::Normalizer.normalize("！”＃＄％＆’（）＊＋，−．／：；＜＞？＠［￥］＾＿｀｛｜｝")
        ...
        assert_equal "南アルプスの天然水Sparking Lemonレモン一絞り", Neologdish::Normalizer.normalize("南アルプスの　天然水　Ｓｐａｒｋｉｎｇ　Ｌｅｍｏｎ　レモン一絞り")
        assert_equal "南アルプスの天然水-Sparking*Lemon+レモン一絞り", Neologdish::Normalizer.normalize("南アルプスの　天然水-　Ｓｐａｒｋｉｎｇ*　Ｌｅｍｏｎ+　レモン一絞り")
      end
    end
  end
end
